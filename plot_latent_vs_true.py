...
def plot_vs_gt_shapes(vae, shapes_dataset, save, z_inds=None):
    ...
    if len(active_units) == 0:
        print("Warning: No active latent units found, skipping plot.")
        return  # stop plotting to avoid matplotlib warnings
...
# # VAR_THRESHOLD = 0.01  # threshold for active latent units, adjust if needed
# def plot_vs_gt_concon(vae, concon_dataset, save_path, z_inds=None):

#     ...

def plot_vs_gt_concon(vae, concon_dataset, save_path, z_inds=None):
    if isinstance(concon_dataset, torch.utils.data.DataLoader):
        raise ValueError("You passed a DataLoader. Pass concon_dataset = dataset_loader.dataset instead.")

    # if isinstance(concon_dataset, DataLoader):
    # raise ValueError("Expected a Dataset, got a DataLoader. Pass dataset_loader.dataset instead.")
    vae.eval()
    dataset_loader = DataLoader(concon_dataset, batch_size=1000, num_workers=1, shuffle=False)

    N = len(dataset_loader.dataset)
    K = vae.z_dim
    nparams = vae.q_dist.nparams  # number of params per latent dim, usually 2 (mean, logvar)

    qz_params = torch.Tensor(N, K, nparams)

    n = 0
    # for xs, _, _ in dataset_loader:
    # for xs, _ in dataset_loader:
    for xs, labels, env_ids in dataset_loader:
        batch_size = xs.size(0)
        # with torch.no_grad():
        ...
        # xs = Variable(xs.view(batch_size, 1, 64, 64).cuda(), volatile=True)
        qz_params[n:n + batch_size] = vae.encoder.forward(xs).view(batch_size, K, nparams).data
        n += batch_size

    # qz_params shape: (N, K, nparams)
    qz_means = qz_params[:, :, 0]

    # Compute variance of each latent dimension across dataset to find active units
    var = torch.std(qz_means, dim=0).pow(2)
    active_units = torch.arange(0, K)[var > VAR_THRESHOLD].long()
    print('Active units: ' + ','.join(map(str, active_units.tolist())))
    n_active = len(active_units)
    print(f'Number of active units: {n_active}/{K}')
    ...
    if len(z_inds) == 0:
        print("No active latent dimensions found. Skipping plot.")
        return  # Or exit the function gracefully

    # Visualize latent means as a heatmap
    fig, ax = plt.subplots(figsize=(12, 6))
    cax = ax.imshow(qz_means[:, z_inds].cpu().numpy().T, aspect='auto', interpolation='nearest', cmap='coolwarm')
    ax.set_xlabel('Data index')
    ax.set_ylabel('Latent dimensions (active)')
    ax.set_title('Latent means across dataset samples')
